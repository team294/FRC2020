...
public class RobotContainer {
  ...
  public void teleopPeriodic() {
    if (!rumbling && intake.intakeGetPercentOutput() == Math.abs(Constants.IntakeConstants.intakeDefaultPercentOutput)) {
      setXBoxRumble(1);
    } else if (rumbling && intake.intakeGetPercentOutput() != Math.abs(Constants.IntakeConstants.intakeDefaultPercentOutput)) {
      setXBoxRumble(0);
    }

    if (!rumbling && limeLight.seesTarget() && Math.abs(limeLight.getXOffset()) <= 1) {
      setXBoxRumble(0.5);
    } else if (rumbling && (!limeLight.seesTarget() || Math.abs(limeLight.getXOffset()) <= 1)) {
      setXBoxRumble(0);
    }
  }
}
